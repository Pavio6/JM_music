...
public class SingerInfoServiceImpl extends ServiceImpl<SingerInfoMapper, SingerInfo>
    ...
    public List<SongBasicInfoVo> searchBySingerId(Long singerId) {
        ...
        List<SongBasicInfoVo> songBasicInfoVos = songInfoMapper.selectTopSongsBySingerId(singerId);
        for (SongBasicInfoVo songBasicInfoVo : songBasicInfoVos) {
            UserFavorite userFavorite = userFavoriteMapper.selectOne(new LambdaQueryWrapper<UserFavorite>()
                    .eq(UserFavorite::getUserId, SecurityUtils.getUserId())
                    .eq(UserFavorite::getTargetType, TargetType.SONG.getValue())
                    .eq(UserFavorite::getTargetId, songBasicInfoVo.getSongId()));
            songBasicInfoVo.setIsLiked(userFavorite != null);
        }
        return songBasicInfoVos;
